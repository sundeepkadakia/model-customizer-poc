...
The product frontend now supports Firebase email/password accounts, individual or company registration, password reset, protected customization routes, and organization-based project access.

1. In Firebase Console for `modelcustomizerplatform`, enable **Authentication → Sign-in method → Email/Password**.
2. Register a Firebase Web App and copy its configuration values into `frontend/.env` using `frontend/.env.example`.
3. Deploy the included Firestore rules:

```bash
firebase deploy --only firestore:rules --project modelcustomizerplatform
```

4. Rebuild and deploy the frontend.
5. Rebuild and deploy the Cloud Run backend because `firebase-admin` and token verification were added.

The Cloud Run service account uses Application Default Credentials to verify Firebase ID tokens. Projects and jobs are no longer publicly accessible. See `docs/FIRESTORE_SCHEMA.md` for the user, organization, membership, project, job, and invitation model.
